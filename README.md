...
1. 고객이 외부 웹 사이트를 통해 단말기 회선의 오더를 요청한다
  1-1. 오더에는 개통,정지,정지복구,해지,해지복구가 있다.
  1-2. 개통을 할때는 빌링팀으로부터 상품 정보를 받아와 정해진 상품으로만 가입가능하게 제공한다.
  1-3. 오더 상태가 변경될때마다 고객에서 sms를 전송한다.
  1-4. 고객은 회선 상태를 조회할 수 있다.
2. 개통이 되면 과금 파일이 들어오면 해당 고객정보를 찾아 레이팅을 하여 데이터 사용량을 누적한다.
3. 사용량이 누적되면 월 말마다 빌링팀은 해당 고객의 요금을 사용량에 따라 계산한다.
  3-1. 중간에 고객의 회선이 정지 및 해지되면 일할계산한다.
4. 계산이 끝난 요금들은 확정된 요금들이며 고객들에게 청구서를 만들어 제공한다.
5. 청구서까지 제공되면 정산팀은 정산을 통해 가입자 수, 사용량 등을 정산한다.
...
2. 장애격리
    1. 오더처리가 원활히 진행되지 않더라도 365일 24시간 받을 수 있어야 한다  Async (event-driven), Eventual Consistency
    1. 결제시스템이 과중되면 사용자를 잠시동안 받지 않고 결제를 잠시후에 하도록 유도한다  Circuit breaker, fallback
3. 성능
    1. 회선의 정보가 바뀔시에는 화면(fronten)에서 조회될 수 있어야한다.  CQRS
    1. 회선 정보가 바뀔때마다 sms을 줄 수 있어야 한다  Event driven
